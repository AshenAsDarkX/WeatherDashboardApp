...
//            dateFormatter.dateFormat = "d MMM yyyy 'at' ha"
    ...
    // Gives us the full day name (e.g., "Monday, Oct 12").
    // For the Forecast list where we need more detail.
    static func formattedDayName(from timestamp: Int) -> String {
            let date = Date(timeIntervalSince1970: TimeInterval(timestamp))
            let dateFormatter = DateFormatter()
            dateFormatter.dateFormat = "EEEE, MMM, d" // Full day name (e.g., "Monday")
            return dateFormatter.string(from: date)
        }
